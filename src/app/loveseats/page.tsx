...
export default function Loveseats() {
  ...
  const handlePrev = () => setCurrent((prev) => (prev === 0 ? loveseatImages.length - 1 : prev - 1));
  const handleNext = () => setCurrent((prev) => (prev === loveseatImages.length - 1 ? 0 : prev + 1));
      ...
      <main>
        <section className="relative w-full min-h-[60vh] md:min-h-[80vh] flex flex-col items-center justify-center overflow-hidden mt-4 md:mt-24">
          <div className="absolute inset-0 w-full h-full z-0">
            <img
              src={loveseatImages[current]}
              alt={`Loveseat Image ${current + 1}`}
