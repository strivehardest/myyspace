...
export default function Sectionals() {
  ...
  const handlePrev = () => setCurrent((prev) => (prev === 0 ? sectionalImages.length - 1 : prev - 1));
  const handleNext = () => setCurrent((prev) => (prev === sectionalImages.length - 1 ? 0 : prev + 1));
      ...
      <main>
        <section className="relative w-full h-screen flex flex-col items-center justify-center overflow-hidden mt-16 md:mt-24">
          <div className="absolute inset-0 w-full h-full z-0">
            <img
              src={sectionalImages[current]}
              alt={`Sectional Image ${current + 1}`}
